...
public class DropComponentProgramStarterMove : DropComponent<Program>
{
    ...
    static bool RandomWeaponFilter(Program p) => ProgramFilters.IsBlue(p) && ProgramFilters.GivesMoveAction(p);

    // Filter out all programs that don't have the correct color and the proper attributes
    static bool Filter(Program p) => ProgramFilters.HasAttributes(p, Program.Attributes.Starter) && ProgramFilters.IsBlue(p);
}
