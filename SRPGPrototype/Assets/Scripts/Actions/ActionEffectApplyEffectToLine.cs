...
public class ActionEffectApplyEffectToLine : ActionEffect, IDamagingActionEffect
{
    ...
    {
        if (!user.Pos.IsOnGridLine(targetData.targetPos))
            return;
        var pos = user.Pos;
        var direction = pos.DirectionTo(targetData.targetPos);
        pos += direction;
        while(pos != targetData.targetPos && grid.IsLegal(pos))
        {
    ...
    {
        return user.Pos.IsOnGridLine(targetData.targetPos);
    }
